...
class BTMessenger:
        ...
        if key == "default_config":
            ...
            for p in sys.path:
                if "bttackler" in p:  ###
                    d = p
                    break
            file_path = os.path.join(d, "default_config.yaml")
    ...
    def write_json_info(self, info_dict, key):
        ...
        logger.info('write json info into %s for key %s' % (file_path, key))
    ...
    def read_json_info(self, key):
        file_path = self.get_file_path(key)
        logger.info("read json info from %s for key %s" % (file_path, key))
    ...
    def if_assessor_send_stop(self):
    ...
    def if_enable(self, lst):
        if self.enable_dict == None:
            self.enable_dict = self.read_advisor_config()["shared"]["enable_dict"]
        for name in lst:
