...
class BTAdvisor(MsgDispatcherBase):
    def __init__(self, seed=None, **kwargs):
        ...
        BTMessenger().write_advisor_config(self.config_dict)
    ...
    def if_component_in_config(self, name):
    ...
    def complete_config_by_default(self):
        ...
        default = BTMessenger().read_default_config_info()
        ...
        for name in self.component_name_list:
            if name in self.config_dict:
                if not self.if_component_in_config(name):  # nni tuner
                    continue
        ...
        if self.if_component_in_config("assessor"):
            self.config_dict["assessor"].update({"codeDirectory": "./"})
            self.config_dict["assessor"].update({"className": "bridger.btassessor.BTAssessor"}) ####
            ...
            for name in self.component_name_list:
                if name in self.config_dict:
                    if name in ["tuner", "assessor"]:
                        if self.if_component_in_config(name):
